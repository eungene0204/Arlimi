...
public class SettingPopUp
{
	...
	public SettingPopUp(Context context, String text, int viewResource )
	{
		mContext = context;
		...
		setContentView(inflater.inflate(viewResource, null));
		
		mHelpTextView = (TextView) mView.findViewById(R.id.popup_text);
		mUpImageView = (ImageView) mView.findViewById(R.id.siva_arrow_down);
		mDownImageView = (ImageView) mView.findViewById(R.id.siva_arrow_up);
...
//		mHelpTextView.setSelected(true);
	...
	{
		this(context,"", R.layout.event_sort);
	}
	
	public SettingPopUp(Context context, String text)
	{
		this(context);
...
/*		
		...
		if( anchorRect.top < rootHeight / 2)
		{
		...
		whichArrow = ((onTop) ? R.id.siva_arrow_down : R.id.siva_arrow_up);
		requestedX = anchorRect.centerX();
		
		View arrow = whichArrow == R.id.siva_arrow_up ? mUpImageView
				: mDownImageView;
		View hideArrow = whichArrow == R.id.siva_arrow_up ? mDownImageView
				: mUpImageView;
		
		whichArrow = R.id.siva_arrow_down;
		arrow = mDownImageView;
		...
		int myX = anchorRect.left - 50;
		int myY = anchorRect.centerY();
		
		
		mWindow.showAtLocation(anchor, Gravity.NO_GRAVITY, myX, myY);
	...
	public void setContentView(int resource)
	{
		...
		setContentView(inflater.inflate(resource, null));
	}
	
	public void setOnDismissListener(PopupWindow.OnDismissListener liseter)
	{
		mWindow.setOnDismissListener(liseter);
	}
